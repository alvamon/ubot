...
import org.ubot.api.methods.GE_Item;
...
public class Inventory {
	...
	public boolean isSelected() throws InterruptedException{
		return (cm.checkColour(Constants.getScreenArea(), new Color(230,157,57), new Point(657,203),20));
	}
	...
	/**
	 * Method to click the selected slot.
	 * 
	 * Use theMouse's button mask's for eg.
	 * 
	 * LEFT_CLICK = 1
	 * RIGHT_CLICK = 3
	 * 
	 ...
	 * @throws InterruptedException
	 */
	public void clickSlot(int slotID, int button_mask) throws InterruptedException{
	 ...
	 * @throws InterruptedException
	 */
	public int findItem(GE_Item item, int tolerance) throws InterruptedException{
		for(int i = 0; i < slots.length; i++){
			if(cm.findImage(cm.getImage(slots[i]), item.getImage(), 0, 0, slotWidth, slotHeight, tolerance))
				return i +1;
		}
	 ...
	 * @throws InterruptedException
	 */
	public ArrayList<Integer> findAllItems(GE_Item item, int tolerance) throws InterruptedException{
	 ...
	 * @throws InterruptedException
	 */
	public int findItemNote(GE_Item item, int tolerance) throws InterruptedException{
		for(int i = 0; i < slots.length; i++){
			if(cm.findImage(cm.getImage(slots[i]), item.getNoteImage(), 0, 0, slotWidth, slotHeight, tolerance))
				return i +1;
		}
