...
import org.ubot.api.utils.Timer;
import org.ubot.connectors.GrandExchangeConnector;
...
public class GrandExchange {
	...
	private String[] disallowedCharacters = new String[]{"'",",","-","(",")"};
	...
	private  Rectangle buttonClose = new Rectangle(477,34,15,15);
	...
	public  boolean isOpen() throws InterruptedException{
		return (cm.similarColours(cm.getColour(new Point(210,40)), new Color(204,152,0), 20));
	}
	...
	public  boolean isBuyScreenOpen() throws InterruptedException{
		return (cm.similarColours(cm.getColour(new Point(187,75)), new Color(200,238,0), 20));
	}
	...
	public  boolean isSellScreenOpen() throws InterruptedException{
		return (cm.similarColours(cm.getColour(new Point(187,73)), new Color(254,174,0), 20));
	}
	...
	public  int findEmptySlot() throws InterruptedException{
		if(cm.similarColours(cm.getColour(new Point(86,90)), new Color(204,153,0), 20) &&
				cm.similarColours(cm.getColour(new Point(120,101)), new Color(204,153,0), 20)){
			return 1;
		}else if(cm.similarColours(cm.getColour(new Point(241,90)), new Color(204,153,0), 20) &&
				cm.similarColours(cm.getColour(new Point(274,101)), new Color(204,153,0), 20)){
			return 2;
		}else if(cm.similarColours(cm.getColour(new Point(397,90)), new Color(204,153,0), 20) &&
				cm.similarColours(cm.getColour(new Point(432,101)), new Color(204,153,0), 20)){
			return 3;
		}else if(cm.similarColours(cm.getColour(new Point(86,211)), new Color(204,153,0), 20) &&
				cm.similarColours(cm.getColour(new Point(120,219)), new Color(204,153,0), 20)){
			return 4;
		}else if(cm.similarColours(cm.getColour(new Point(241,211)), new Color(204,153,0), 20) &&
				cm.similarColours(cm.getColour(new Point(276,211)), new Color(204,153,0), 20)){
			return 5;
		}else if(cm.similarColours(cm.getColour(new Point(397,211)), new Color(204,153,0), 20) &&
				cm.similarColours(cm.getColour(new Point(432,219)), new Color(204,153,0), 20)){
			return 6;
		}
	...
	public  boolean searchItem(String item) throws InterruptedException{
		...
		while(!cm.similarColours(cm.getColour(new Point(490,460)), new Color(143,127,100), 20)){
			if(t.isUp())
				break;
		...
		cm.theKeyboard.sendKeys(item, true);
		cm.sleep(1500,2000);
		
		t.reset();
		while(cm.similarColours(cm.getColour(new Point(370,365)), new Color(160,90,0), 20)){
			if(t.isUp())
				break;
		...
		for(int i = 0; i < 8; i++){
			...
			String s = cm.textReader.getText(r, OCRFont.GE_SMALL_TEXT, 45);
			
			for(String a : disallowedCharacters){
				s = s.replace(a, "");
			}
			
			if(s.equals(newItem)){
				for(int a = 0; a < 3; a++){
					cm.theMouse.click(cm.getRandomPoint(new Rectangle((int)r.getX(),(int)r.getY(),200,14)),cm.theMouse.LEFT_CLICK);
					cm.sleep(1500,2000);
					
					if(!cm.textReader.getText(new Rectangle(52,135,150,16), OCRFont.GE_STAT_TEXT, 45).contains("N/A"))
						return true;
				}
	...
	public  void setQuantity(int Quantity) throws InterruptedException{
		if(Quantity > 10000){
			...
			while(!cm.similarColours(cm.getColour(new Point(327,406)), new Color(0,0,0), 20))
				cm.sleep(100,200);
		...
		for(Rectangle r : collectBoxs){
			if(isBuyScreenOpen() || isSellScreenOpen())
				if(!cm.allSimilarColours(new Color(69,56,36), cm.getColours(r), 30)){
					cm.theMouse.click(cm.getRandomPoint(r),cm.theMouse.LEFT_CLICK);
					cm.sleep(300,500);
				}
		}
	}
